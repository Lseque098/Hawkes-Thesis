...
sim_Hawkes_Euler<- function(lambda0, a, beta, alpha, fin_data, n)
  ...
  lambda0 = 1.5 #intensidad inicial
a = .8       # parametro de reversion
beta = 5    #tasa de decaimiento
alpha = .8   # magnitud del salto
Tiempo = 10      # fin_data
n = 250*Tiempo 
...
library(ggplot2)
ggplot(df,aes(x=TarriboP))+geom_line(aes(y=N))+ggtitle("Proceso de Hawkes")+theme(plot.title = element_text(hjust=.5))+
  xlab("Tiempo de Arribo")+ylab("# de Arribos")+xlim(0,Tiempo)+ylim(0,n_saltos)+scale_y_continuous(breaks=seq(0,n_saltos, by=2))+scale_x_continuous(breaks = seq(0,Tiempo,by=2))
